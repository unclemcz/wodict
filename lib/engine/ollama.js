...
async function translate(query,engine,win,controller) {
    ...
    if (key == '' || key == null || key == undefined) {
        ...
        result.resulttext = '请安装ollama与模型，并启动ollama服务。';
    ...
    const reqdata = {
        ...
        "prompt": "Translate the following text to chinese:"+query+".Output translation directly without any additional text.",
        //"stream": false
        "think":false
    }
        ...
        .then(response => {
            ...
            parser.on('data', data => {
                ...
                if (!data.done) {
                    ...
                    result.resulttext = result.resulttext.replace(/<think>[\s\S]*<\/think>/i, '');
                    result.done = true;
                }
                win.webContents.send('update-text', result);
                //console.log(result);
